    	...
    	if(!isYourTurn) {
    		...
    		if(isYourTurn) {
    	    	...
    	    	for(int i = 0; i < 16; i++) {
    	    		...
    	    		if(g.board.tables[i/4].table[i%4] == 1) tile.setImageResource(R.drawable.sample_1);
    	    		if(g.board.tables[i/4].table[i%4] == 2) tile.setImageResource(R.drawable.sample_2);
    	    	}
    		}
    		return;
    	}
